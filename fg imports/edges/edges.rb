...
edges = doc.xpath("//reference/edges").first.elements
#edges = doc.css("edges").first.elements

# puts edges.count
# exit

puts 'Name,Requirements,Type,Benefit'
0.upto(edges.count-1) do |e|
  record = '"' + edges[e].at_css('name').content + '",'
  #puts edges[e].at_css('name').content
  record = record + '"' + edges[e].at_css('prerequisites').content + '",'
  record = record + '"' + edges[e].at_css('type').content.gsub(/"/, '\"') + '",'
  #record = record + '"' + edges[e].at_css('shortdescription').content.gsub(//, '–').gsub(//, '—').gsub(//, '“').gsub(//, '”').gsub(//, '’').gsub(/"/, "”") + '",'
  record = record + '"'
  print record
